  ...
  grunt.registerMultiTask('uglify', 'Minify files with UglifyJS.', function() {
    ...
    this.files.forEach(function(f) {
      ...
      for(var i=0;i<srcArr.length;i++){
        var src=[srcArr[i]];
        var sArr=srcArr[i].split("/");
        var fileName=sArr[sArr.length-1];
        var result;
        try {
          result = uglify.minify(src, f.dest+"/"+fileName, options);
        } catch (e) {
          console.log(e);
        ...
        grunt.file.write(f.dest+"/"+fileName, output);
        ...
        grunt.log.writeln('File ' + chalk.cyan(f.dest)+'/'+fileName + ' created.');
